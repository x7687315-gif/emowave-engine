...
## 安全审查（P3C 规范，第二轮 · 安全聚焦）

用 `p3c-code-quality`（阿里巴巴 P3C 规范，按 Python 适配其安全/异常/并发/DB/参数校验类别）对 `emowave/` 做了第二轮安全聚焦复查。

### 扫描结果（干净）

- **SEC-001 SQL 注入**：全参数化查询（`?` 占位），零字符串拼接。`_install_immutable_triggers` 的 f-string 仅用硬编码表名常量，非用户输入。
- **SEC-002 敏感数据脱敏**：6 处日志调用逐一核对，**均不泄露用户文本或情绪数值**（只记异常消息/超时/降级原因）。符合 §26 隐私原则。
- **SEC-004 硬编码密钥**：内核零命中；GitHub 令牌从未写入任何提交文件（仅 push 命令行内联）。唯一 `password=` 命中在 1.x `privacy_export.py` 测试夹具（假占位符）。
- **危险调用**：无 `eval`/`exec`/`os.system`/`subprocess`/`pickle.load`/`yaml.load`/`shell=True`。
- **异常纪律**：宽 `except Exception` 仅在文档化的降级路径（带日志）与平台探测（安全默认）；无空 catch；核心数值路径只捕获窄 `ValueError`。
- **可变默认参数陷阱**：零命中。
- **core 层 print**：零（print 仅在 CLI，合法）。

### 本轮补修（2 项，全局性完成上次未竟）

- **[MEDIUM 补全] SQLite 读方法未串行化**：上轮只锁了写方法，但同一连接上**读写并发同样不安全**（且上轮并发测试只测了并发写，给了假信心）。本轮给全部 11 个读方法补 `@_synchronized`，读写彻底串行化；并把并发测试升级为**读写混合**（6 写线程 + 6 读线程）以真正验证。
- **[LOW] CLI `--points` 无界**：误输入超大值会内存/时间爆炸。加 `MAX_SYNTH_POINTS=200000` 钳制（SEC-003 参数有效性）。

### 文档更新

README 重写为**简约 / 构成主义**风格：新增 `docs/emowave-banner.svg`（红楔 + 墨黑对角 + 纸白 + 粗黑无衬线的 El Lissitzky 式 banner），编号分节（01·定位 … 12·回滚），有限色板徽章（纸白/墨黑/一色红），技术要点（四层架构、Matérn 闭式解、安全隐私表、降级、性能、CLI）与视觉并重。

**测试总数 606 全通过**（含新增读写混合并发测试）。
